...
const Enterdetail = () => {
  ...
  useEffect(() => {
    if (carId) {
      axios.get(`https://cr-backend-ydia.onrender.com/car/${carId}`)
  ...
  useEffect(() => {
    if (submitSuccess && formData.email) {
      axios.get('https://cr-backend-ydia.onrender.com/detail/detail')
        .then(res => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('https://cr-backend-ydia.onrender.com/api/buycar', data, {
        ...
        <div className='px-8 py-6 border-b border-gray-200'>
          ...
          {car && (
            <div className='mt-6 flex flex-col md:flex-row items-center gap-6 justify-center bg-blue-50 rounded-xl p-4'>
              <img src={`https://cr-backend-ydia.onrender.com${car.image}`} alt={car.name} className='w-32 h-20 object-contain rounded-lg shadow' />
              <div>
